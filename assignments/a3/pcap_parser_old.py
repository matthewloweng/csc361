...
class PcapParser:
    ...
    def parse(self):
            ...
            while True:
                ...
                if eth_header[-2:] == b'\x08\x00':  # IPv4
                    ...
                    if protocol == 1:  # ICMP protocol
                        ...
                        if icmp_type == 8:
                            _, icmp_seq = struct.unpack(
                                '!HH', icmp_packet[4:8])
                            # store send times for windows traces
                                ...
                                if encapsulated_protocol == 17:  # Encapsulated UDP
                                    ...
                                    if len(encapsulated_ip_packet) >= encapsulated_header_length + 8:
                                        ...
                                        if 33434 <= dest_port <= 33529:
                                            ...
                                            orig_ttl = encapsulated_ip_packet[8]
        ...
        for datagram_id, fragment_details in self.fragments.items():
            print(
                f"The number of fragments created from the original datagram {datagram_id} is: {fragment_details['count']}")
            print(
                f"The offset of the last fragment is: {fragment_details['last_offset']}")
